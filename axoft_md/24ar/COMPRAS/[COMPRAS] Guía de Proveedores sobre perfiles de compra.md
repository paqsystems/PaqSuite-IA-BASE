# Guía de Proveedores sobre perfiles de compra
...
- Modulo: Compras
- Categoria: Guía
- URL: https://ayudas.axoft.com/24ar/documentos/guias/guias_carp_cp/guia_perfiles_cp/
...
Al definir un perfil de usuario, es posible configurar los permisos sobre cualquiera de las siguientes operaciones:
  ...
  * [Factura de compra](https://ayudas.axoft.com/24ar/comprobfactura_cp)
  * [Autorización de comprobantes](https://ayudas.axoft.com/24ar/ccorrautcomprpagar_cp)



Cabe mencionar que la definición de perfiles no es obligatoria. Si un usuario no tiene asignado ningún perfil, entonces estarán disponibles las funciones del módulo Proveedores que se hayan configurado en [Parámetros de Proveedores](https://ayudas.axoft.com/24ar/paramgrales_cp).
...
**A continuación se expone un ejemplo en el que se detallan los parámetros más importantes a configurar en cada uno de los perfiles, definidos según el rol que desempeñan los usuarios: 
...
**Es posible crear un perfil genérico, que sea utilizado para que todos los empleados ingresen [Solicitudes de compra](https://ayudas.axoft.com/24ar/perfsolicitudcp_cp) correspondientes a los insumos de librería. Este perfil le permitirá agilizar y restringir la carga de datos, que realicen los usuarios para este tipo de solicitud.
...
_A continuación se detallan las funcionalidades disponibles para el usuario asociado a este perfil.
  ...
  * Edita la clasificación definiendo si la solicitud es, por ejemplo, de prioridad alta, media, urgente o baja. Consulte cómo definir una [clasificación](https://ayudas.axoft.com/24ar/clasificacion1_cp) para cada tipo de comprobante.
...
Para más información consulte el tópico [Perfiles de solicitud de compra](https://ayudas.axoft.com/24ar/perfsolicitudcp_cp).

**Perfiles de orden de compra  
**Es posible crear un perfil que sea utilizado para el comprador, que realiza las compras de insumos de librería, y otro para el jefe de compras, responsable de autorizar todas las ordenes emitidas y de generar otros tipos de ordenes. De esta forma obtendremos para el primer caso, un ingreso de [orden de compra](https://ayudas.axoft.com/24ar/ordencpgeneracion_cp) ágil y restrictivo, y para el segundo caso, una carga más completa y flexible.
...
Para más información consulte el tópico [Perfil de Orden de Compras](https://ayudas.axoft.com/24ar/perfordencp_cp).

**Perfiles de factura de compra  
**Se puede crear un perfil que sea utilizado para el operador que ingresa habitualmente todas las facturas de proveedores y otro para el jefe de compras quien tiene a cargo la supervisión del sector, de esta forma obtendremos para el primer caso una carga de [facturas de compra](https://ayudas.axoft.com/24ar/comprobfactura_cp) mas ágil y restrictiva; y para el segundo caso, una carga completa y flexible.
...
Para más información consulte el tópico [Perfiles de factura de compra](https://ayudas.axoft.com/24ar/perffacturacp_cp).
