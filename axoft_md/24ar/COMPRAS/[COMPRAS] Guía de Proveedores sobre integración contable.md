# Guía de Proveedores sobre integración contable
...
- Modulo: Compras
- Categoria: Guía
- URL: https://ayudas.axoft.com/24ar/documentos/guias/guias_carp_cp/guia_integrcontabl_cp/
...
Al integrar con el módulo Contabilidad, usted debe definir algunos datos necesarios y obligatorios. Otros valores son opcionales, aunque ayudan a que la información contable sea más rica a la hora de realizar análisis contables.
  ...
  * Complete los [parámetros contables](https://ayudas.axoft.com/24ar/paramcontparam_cp) del módulo Compras, si genera asiento con el ingreso del comprobante o lo genera luego, etc.
  * Ingrese los [modelos de asientos](https://ayudas.axoft.com/24ar/paramcontmodasiento_cp) y la parametrización contable de los [tipos de comprobantes](https://ayudas.axoft.com/24ar/paramcontipocomprob_cp).
  * En forma opcional, usted puede definir parámetros contables para [proveedores,](https://ayudas.axoft.com/24ar/paramcontproveed_cp) artículos, [conceptos](https://ayudas.axoft.com/24ar/paramcontconcepcp_cp) o [para tipo de gastos](https://ayudas.axoft.com/24ar/paramcontipogasto_cp).
...
**En caso de activar el parámetro Genera asiento en el ingreso del comprobante en [parámetros contables](https://ayudas.axoft.com/24ar/paramcontparam_cp) del módulo Compras, al ingresar un nuevo comprobante de factura, nota de débito o nota de crédito, se generará el asiento correspondiente.

  * Si es posible modificar el asiento en el ingreso: porque el [perfil de facturación](https://ayudas.axoft.com/24ar/perffacturacp_cp) o el [tipo de comprobante](https://ayudas.axoft.com/24ar/paramcontipogasto_cp) lo permite, al terminar la carga del comprobante se abrirá automáticamente la pantalla del asiento, usted podrá consultar el asiento generado, realizar modificaciones e informar las apropiaciones en los casos que se requiera.  
El sistema completa esta pantalla tomando los datos del [modelo de asiento](https://ayudas.axoft.com/24ar/paramcontmodasiento_cp), o tomando los datos de artículos, artículos con escalas, [conceptos](https://ayudas.axoft.com/24ar/paramcontconcepcp_cp), [proveedores](https://ayudas.axoft.com/24ar/paramcontproveed_cp).  
...
El sistema completa esta pantalla:
  ...
  * Toma las apropiaciones de artículos, artículos con escalas, [conceptos](https://ayudas.axoft.com/24ar/paramcontconcepcp_cp) o [proveedores](https://ayudas.axoft.com/24ar/paramcontproveed_cp).
  * Toma las apropiaciones del [modelo de asiento](https://ayudas.axoft.com/24ar/paramcontmodasiento_cp).
...
Para poder generar los asientos pendientes ejecute el proceso [Generación de asientos contables.](https://ayudas.axoft.com/24ar/generasientcont_cp) Este proceso filtra los comprobantes con asientos sin generar, y los genera en forma masiva, teniendo en cuenta el modelo de asiento asociado al comprobante y la configuración de artículos, artículos con escalas, conceptos, proveedores.  
...
Para más información sobre configuración de parámetros contables consulte [Parámetros contables](https://ayudas.axoft.com/24ar/paramcontparam_cp), [Tipos de comprobantes](https://ayudas.axoft.com/24ar/tipocomprobante_cp), [Perfiles de facturas de compras](https://ayudas.axoft.com/24ar/perffacturacp_cp).  
...
_Una vez ingresado el comprobante, usted puede acceder a la pantalla de asientos desde la opción [Modificación de comprobantes](https://ayudas.axoft.com/24ar/modificomprob_cp) para:

  * Modificar el asiento generado en el ingreso de comprobante.
  * Generar el asiento pendiente, porque se canceló en el ingreso del comprobante o porque se realiza con posterioridad al ingreso con el proceso [Generación de asientos contables.](https://ayudas.axoft.com/24ar/generasientcont_cp)
...
_Acceda a la misma desde [Modificación de comprobantes](https://ayudas.axoft.com/24ar/modificomprob_cp) o desde una consulta Live, donde encontrará información relacionada con: artículos, vencimientos, imputaciones contables, valores y totales, entre otros.  
...
**Puede realizar la [eliminación de asientos contables](https://ayudas.axoft.com/24ar/elimiasientcont_cp) del módulo Compras desde la opción Contabilización disponible desde Procesos periódicos, utilizando los siguientes criterios de selección: por fecha de emisión o contable, comprobantes con asiento generado o exportado.  
...
**Una vez generados los asientos de todos los comprobantes, está en condiciones se realizar la [Exportación de asientos contables](https://ayudas.axoft.com/24ar/exportasientcontab_cp) al módulo Contabilidad.  
