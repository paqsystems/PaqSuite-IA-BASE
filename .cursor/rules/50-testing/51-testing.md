...
### Requisitos para E2E (Login, Grupos empresarios, etc.)
Los tests E2E que involucran login o API requieren:
1. **Backend en ejecución:** `cd backend && php artisan serve` (puerto 8000)
2. **Base de datos migrada y con seeders:** `php artisan migrate` y `php artisan db:seed`
3. **Credenciales de la aplicación (tabla USERS):** el helper `loginAsE2EUser` prueba en orden ADMIN/admin123, EMP/emple123, PQ/Paqsystems (o `E2E_USER_CODIGO` / `E2E_USER_PASSWORD` si están definidos). Ver `docs/06-operacion/deploy-infraestructura.md`.
4. **Conexión al motor SQL Server:** en `backend/.env` siguen siendo **`DB_USERNAME=Axoft`** y **`DB_PASSWORD=Axoft`** (no son usuarios de login web).

Si el backend no está disponible, los tests de login fallarán (permanecen en /login).

### Partes Producción – Turnos (TR-014 / TR-015)
En `partes-produccion-turnos.spec.ts` hay un caso que crea un turno de **00:00** a **24:00** y verifica la grilla (TR-015 update 02).
