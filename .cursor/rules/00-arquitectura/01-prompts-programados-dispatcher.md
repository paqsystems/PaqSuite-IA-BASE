...
## Objetivo
Esta regla define el flujo **oficial** del proyecto: **siempre Open-Spec completo** (SPEC → HU → TR → ejecución → tests → verificación). Los comandos abreviados evitan copy/paste y aseguran trazabilidad.
...
Ejecutar **en este orden** (prompts `openspec-01` … `openspec-04` en `prompts/`):
...
| 5 | **E** | Tests automáticos (backend + unitarios + E2E). |
| 6 | **F** | Verificación documental: `prompts/openspec-04-verificar-implementacion.md`. |
...
3. **PARTE E** — Tests.
4. **PARTE F** — `openspec-04` sobre el TR afectado.
...
| **F** | Verificación docs ↔ código (**openspec-04**) | Ver **PARTE F** |
...
4. Leer el archivo de prompt:
   `prompts/05-Ejecucion-de-una-TR.md` **o** `docs/prompts/05-Ejecucion-de-una-TR.md` (según exista en el repo).
...
## PARTE F — Verificación implementación (**openspec-04**)

### Comandos (ejemplos)

- `Verificá la implementación contra el TR docs/04-tareas/...`
- `Ejecutá openspec-04 para el TR-…`

### Comportamiento

1. Ejecutar **`prompts/openspec-04-verificar-implementacion.md`** con la ruta del **TR** (obligatoria); SPEC/HU desde enlaces del TR si existen.
...
0. **Open-Spec — cambio de alcance (obligatorio si cambia el “qué”):** Si una solicitud del control altera comportamiento funcional, reglas de negocio, exclusiones, criterios de aceptación o mensajes pactados, **antes** de cualquier HU-update/TR-update el asistente debe:
   ...
   - Crear o actualizar el **SPEC-update** en **`docs/05-open-spec/updates/<subcarpeta>/`** con el **mismo nombre base** que el SPEC original más sufijo **`-update`**, **`-update-01`**, etc. (misma **`<subcarpeta>`** relativa que en ruta base; ver **`.cursor/rules/base/00-arquitectura/08-open-spec-gobernanza.md`**). En metadatos del SPEC-update: **`Estado: Pendiente`** y enlace o campo **SPEC base** con la ruta del original; en **Origen**, trazabilidad **`00-ControlCalidad-{sigla}`** y **fecha** (dd/MM/yyyy) como en HU-update.
...
2. **Condición para fusionar cada update:** Solo incorporar un **`HU-...-update*.md`**, **`TR-...-update*.md`** o **`SPEC-...-update*.md`** si ese archivo figura con **`Estado: Finalizado`** en su tabla de metadatos (marcado manualmente; ver `.cursor/rules/base/00-arquitectura/07-estado-hu-tr.md`, sección **6) Unificación**). Los updates con otro **Estado** no se fusionan.
...
8. Generar el archivo TR según las reglas del prompt; en la tabla de metadatos incluir **`Estado: Pendiente`** (ver `.cursor/rules/base/00-arquitectura/07-estado-hu-tr.md`).
...
## Beneficio
Esta regla define el flujo **Open-Spec completo** como **predeterminado**: **A** (`openspec-01`) → **B** (`openspec-02`) → **C** (`openspec-03`) → **D** → **E** → **F** (`openspec-04`). El prefijo **A–F** coincide en orden **alfabético y cronológico**. La **Parte J** encadena **A+B** desde `docs/02-producto/`; la **Parte K** es **excepción** con deuda explícita hacia **A**.
...
Referencias: **`.cursor/rules/base/00-arquitectura/08-open-spec-gobernanza.md`**, **`docs/_base/_OPEN-SPEC-METODOLOGIA.md`**.
